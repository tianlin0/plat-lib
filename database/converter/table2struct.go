...
func (t *Table2Struct) Run() (string, error) {
	...
	allContent := packageName + importContent + structContent
	// 是否指定保存路径
	if t.savePath != "" {
		filePath := fmt.Sprintf("%s", t.savePath)
